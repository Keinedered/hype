...
import { coursesAPI, graphAPI, modulesAPI, tracksAPI } from '../api/client';
import {
  normalizeCourse,
  normalizeGraphEdge,
  normalizeGraphNode,
...
export function KnowledgeGraphPage({ onNodeClick, onOpenHandbook }: KnowledgeGraphPageProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const [rawCourses, rawTracks, rawNodes, rawEdges] = await Promise.all([
          coursesAPI.getAll(),
          tracksAPI.getAll(),
          graphAPI.getNodes(),
          graphAPI.getEdges(),
  ...
  const courseIdFromGraph = useMemo(() => {
    ...
    if (currentNode) return currentNode.entityId;
    const firstCourseNode = nodes.find((node) => node.type === 'course');
    return firstCourseNode?.entityId ?? null;
  }, [nodes]);

  const activeCourse = useMemo(() => {
    if (courseIdFromGraph) {
      return courses.find((course) => course.id === courseIdFromGraph) ?? null;
    }
    return courses[0] ?? null;
  }, [courses, courseIdFromGraph]);

  const activeTrack = useMemo(() => {
    if (!activeCourse) return null;
    return tracks.find((track) => track.id === activeCourse.trackId) ?? null;
  }, [activeCourse, tracks]);
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <h3 className="font-mono tracking-wide mb-1">
                    {activeCourse?.title?.toUpperCase() ?? 'КУРС НЕ ВЫБРАН'}
                  </h3>
