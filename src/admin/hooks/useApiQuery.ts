  ...
  const executeQuery = useCallback(async (skipCache = false) => {
    ...
    if (!skipCache && cache.has(queryKey)) {
      ...
      if (now - cached.timestamp < cached.cacheTime) {
        ...
        const cachedData = cached.data !== null && cached.data !== undefined 
          ? cached.data 
          : [];
        setData(cachedData);
        setLoading(false);
    ...
    const performRequest = async (attempt: number): Promise<void> => {
      try {
        ...
        // Нормализуем результат: если null/undefined, возвращаем пустой массив для списков
        // Это гарантирует, что мы всегда имеем валидную структуру данных
        const normalizedResult = result !== null && result !== undefined 
          ? result 
          : [];
